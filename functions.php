...
namespace wcms20GoogleMapBlock;
...
function load_custom_blocks() {
  ...
  // register_block_type('wcms20/google-map', array(
  //   'editor_script' => 'wcms20-map',
  // ));
}
...
function getGoogleMapAPIKey()
{
	...
	return get_option( 'wcms20_google_map_block_api_key', '' );
}
...
function registerSettings() {
    register_setting(
        'wcms20_google_map_block_api_key',
        'wcms20_google_map_block_api_key',
        array(
            'type' => 'string',
...
function renderGutenbergMapEmbedblock( $attributes ) {
    ...
    $classNames = ( $interactive ) ? "wp-block-wcms20-google-map interactive ratio$aspectRatio" : "wp-block-wcms20-google-map";
    ...
    // Return the output
    return apply_filters('render_wcms20_google_map_block', $output, $attributes);
}
